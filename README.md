...
更完整的**项目概括与三种运行模式规则**见 [PROJECT.md](PROJECT.md)。  
**要点**：主配料在**窗口右上角 ROI** 里做模板匹配（看订单预览里有没有该小图）；命中后再去点 `points.json` 里对应**槽位/补充**坐标（例如黄油槽可在画面别处，与 ROI 无需重合）。`提交1` 在整幅窗口图上匹配，落点由匹配框中心决定，一般不用 JSON。
