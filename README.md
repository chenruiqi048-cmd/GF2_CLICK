...
- 坐标与模板基于 **1600×900** 分辨率标定，更换设备、分辨率或 DPI 缩放后需重新标定
- **关于 DPI 缩放**：若标定与运行时保持相同的显示缩放，坐标是准确的。若标定在 150%、运行在 100%，可在 `gf2_bot.py` 中设置 `COORD_SCALE = 1.5`（标定 DPI / 运行 DPI）实现等比例缩放，无需重新标定
